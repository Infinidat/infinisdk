...
""".format(
        ...
        field,
        _format_type_doc(field.type._type),
    )
    return getter
...
def make_field_updaters(field):
    ...
    if field.type._type is bool:  # pylint: disable=protected-access
        ...
        deprecation_msg = "Use {} instead".format(
            "/".join(func.__name__ for func in updaters_list)
        )
...
def get_data_for_object_creation(object_type, system, fields):
    ...
    for field in object_type.fields:
        ...
        if field_value is not NOTHING and field_api_value is not NOTHING:
            raise ValueError(
                "Multiple colliding arguments: {} and {}".format(
    ...
    if missing_fields:
        raise MissingFields(
            "Following fields were not specified: {}".format(
