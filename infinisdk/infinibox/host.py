...
from ..core.translators_and_types import HostPortListType, host_port_to_api
...
class HostBinder(TypeBinder):
    ...
    def get_host_id_by_initiator_address(self, address):
        ...
        """
        try:
            res = self.system.api.get(
                "hosts/host_id_by_initiator_address/{}".format(address),
                check_version=False,
            )
...
class Host(InfiniBoxLURelatedObject):
    ...
    FIELDS = [
        ...
        Field(
            "is_min_expected_paths_user_set",
            type=bool,
            is_sortable=True,
            feature_name="host_resiliency",
        ),
        ...
        Field(
            "cluster",
            api_name="host_cluster_id",
            type="infinisdk.infinibox.host_cluster:HostCluster",
            is_filterable=True,
            binding=RelatedObjectBinding("host_clusters"),
        ),
        Field(
            "host_type",
            creation_parameter=True,
