...
class ConsGroup(InfiniBoxObject):
    URL_PATH = "cgs"
    ENTITY_TYPES = namedtuple("VolumeTypes", ["Master", "Snapshot"])(
        "MASTER", "SNAPSHOT"
    )
    ...
    def create_snapgroup(
        ...
        if self.system.compat.has_snapshot_lock():
            for key, val in [("lock_expires_at", lock_expires_at)]:
                data[key] = self.fields.get(key).binding.get_api_value_from_value(
                    self.system, type(self), None, val
                )
        ...
        child_members = dict(
            (snapshot.get_parent(from_cache=True).id, snapshot)
            for snapshot in child.get_members()
        )
        for member in members:
    ...
    def refresh_snapgroup(self):
        ...
        sg_members_by_parent_id = dict(
            (s.get_field("parent_id", from_cache=True), s) for s in self.get_members()
        )
        cg_members_by_id = dict(
            (m.id, m) for m in parent.get_members() if m.id in sg_members_by_parent_id
        )
    ...
    def delete(
        self, delete_members=OMIT, force_if_snapshot_locked=OMIT
        ...
        try:
            self._send_delete_with_hooks_triggering(
                self.get_this_url_path(),
    ...
    def get_members(self, **kwargs):
        ...
        def object_factory(system, received_item):
            type_name = (
                "volume" if received_item["dataset_type"] == "VOLUME" else "filesystem"
            )
            return system.objects.get_binder_by_type_name(
                type_name
            ).object_type.construct(system, received_item)

        object_types = (
            self.system.volumes.object_type,
            self.system.filesystems.object_type,
        )
        ret = PolymorphicQuery(
            self.system, self._get_members_url(), object_types, object_factory
        )
        if kwargs:
    ...
    def add_member(self, member, **kwargs):
        ...
        if (
            ...
            and "ACTIVE_ACTIVE" in self.get_replication_types()
        ):
            if remote_entity:
    ...
    def restore(self, snap_group):
        ...
        sg_members_by_parent_id = dict(
            (s.get_field("parent_id", from_cache=True), s)
            for s in snap_group.get_members()
        )
        members_by_id = dict(
            (m.id, m) for m in self.get_members() if m.id in sg_members_by_parent_id
        )
        for parent_id, snap in reversed(list(sg_members_by_parent_id.items())):
            members_by_id[parent_id].trigger_before_restore(snap)
        try:
            self.system.api.post(
                self.get_this_url_path().add_path("restore"),
