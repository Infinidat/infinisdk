...
class PoolBinder(TypeBinder):
    ...
    def get_administered_pools(self):
        ...
        return ObjectQuery(
            self.system, URLObject("/api/rest/pools/administered_pools"), Pool
        )
...
class Pool(InfiniBoxObject):
    ...
    FIELDS = [
        ...
        Field(
            ...
            type=list,
            add_updater=False,
            binding=ListOfRelatedObjectIDsBinding("users"),
        ),
        Field(
            "allocated_physical_capacity",
            api_name="allocated_physical_space",
            type=CapacityType,
        ),
        Field(
            "free_physical_capacity", api_name="free_physical_space", type=CapacityType
        ),
        Field(
            "free_virtual_capacity", api_name="free_virtual_space", type=CapacityType
        ),
        Field(
        ...
        Field(
            "qos_policies",
            type=list,
            binding=ListOfRelatedObjectBinding("qos_policies"),
            feature_name="qos",
            cached=False,
        ),
        Field(
        ...
        Field(
            "standard_filesystem_snapshots_count",
            type=int,
            feature_name="standard_counts",
        ),
    ...
    def _lock_unlock_operation(self, operation_name):
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.pre_pool_{}".format(operation_name),
            {"pool": self},
            tags=hook_tags,
        )
        try:
    ...
    def _is_over_threshold(self, threshold):
        # pylint: disable=no-member
        return (self.get_allocated_physical_capacity() * 100) >= (
            self.get_physical_capacity() * threshold
        )
    ...
    def unassign_qos_policy(self, qos_policy):
        assert self.system.compat.has_qos(), "QoS is not supported in this version"
        assert (
            qos_policy in self.get_qos_policies()
