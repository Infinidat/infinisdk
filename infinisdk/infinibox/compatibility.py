...
class Compatibility:
    ...
    def can_run_on_system(self):
        ...
        restrictions = self._parse_restrictions(
            config.root.infinibox.compatible_versions
        )
    ...
    def has_npiv(self):
        return self._has_feature("fc_soft_targets") or self._has_feature(
            "fc/soft_targets"
        )
    ...
    def has_auth_sessions(self):
        return self._has_feature("api_auth_sessions") or self._has_feature(
            "api/auth_sessions"
        )
    ...
    def has_native_smb_dual_protocol(self):
        return self.has_native_smb() and self.get_parsed_system_version() >= "7.2"
...
class InfiniboxVersion:
    ...
    def parse(cls, version):
        ...
        is_dev = is_odd = False
        parsed_version = tuple(int(ver_digit) for ver_digit in before_dash.split("."))
        ...
        parsed_version += tuple(
            ("*" if index >= 4 else 0)
            for index in range(len(parsed_version), _VERSION_TUPLE_LEN)
        )
    ...
    def __eq__(self, other):
        ...
        if self.version != other_ver.version:
            return False
        if (
            self._is_odd_version or other_ver._is_odd_version
    ...
    def __lt__(self, other):
        ...
        if self.version == other_ver.version:
            if (
                self._is_odd_version
                or other_ver._is_odd_version
                or (self._is_dev and other_ver._is_dev)
            ):
