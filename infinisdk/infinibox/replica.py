...
class ReplicaBinder(TypeBinder):
    ...
    def replicate_cons_group(self, cg, remote_cg=None, remote_pool=OMIT, **kw):
        ...
        :seealso: :meth:`.replicate_entity`
        """
        return self.replicate_entity(
            entity=cg, remote_entity=remote_cg, remote_pool=remote_pool, **kw
        )

    def replicate_entity(
        self, entity, link, remote_pool=None, remote_entity=None, **kw
    ):
        ...
        if remote_entity is None:
            ...
            return self.replicate_entity_create_target(
                entity, link, remote_pool=remote_pool, **kw
            )
        return self.replicate_entity_existing_target(
            entity, link, remote_entity=remote_entity, **kw
        )

    def replicate_entity_create_target(
        self, entity, link, remote_pool=OMIT, remote_entity_names=OMIT, **kw
    ):
        ...
        if remote_entity_names is not OMIT:
            assert isinstance(
                remote_entity_names, (list, tuple)
            ), "object remote_entity_names must be tuple or list"
        return self.system.replicas.create(
            link=link,
            entity_pairs=self._build_entity_pairs_create_target(
                remote_entity_names, entity
            ),
            remote_pool_id=remote_pool.id if remote_pool is not OMIT else OMIT,
            **self._get_extra_replica_kwargs(kw, entity)
        )

    def replicate_entity_existing_target(
        self, entity, link, remote_entity, member_mappings=None, **kw
    ):
        ...
        :param member_mappings: required if remote_entity is specified and is a consistency group. This parameter is
                                a dictionary mapping local member entities to remote ones
        """
        entity_pairs = self._build_entity_pairs_existing(
            entity, remote_entity, member_mappings, use_snapshots=False
        )
        return self.system.replicas.create(
            link=link,
    ...
    def replicate_entity_use_base(
        self, entity, link, local_snapshot, remote_snapshot, member_mappings=None, **kw
    ):
        ...
        """
        entity_pairs = self._build_entity_pairs_existing(
            local_snapshot, remote_snapshot, member_mappings, use_snapshots=True
        )
        creation_kwargs = self._get_extra_replica_kwargs(
            kw, entity, remote_entity=remote_snapshot.get_parent()
        )
        return self.system.replicas.create(
            link=link, entity_pairs=entity_pairs, **creation_kwargs
        )

    def replicate_entity_take_snap(
        self, entity, link, remote_entity, member_mappings=None, **kw
    ):
        ...
        creation_kwargs = self._get_extra_replica_kwargs(
            kw, entity, remote_entity=remote_entity.get_parent()
        )
        return self.system.replicas.create(
            link=link, entity_pairs=entity_pairs, **creation_kwargs
        )
    ...
    def _get_extra_replica_kwargs(self, kw, entity, remote_entity=None):
        ...
        if "base_action" in returned:
            raise InvalidUsageException(
                "replicate_entity() doesn't support creating replica with base_action format"
            )
    ...
    def _build_entity_pairs_create_target(self, remote_entity_names, entity):
        ...
        for sub_entity in self._get_sub_entities(entity):
            ...
            if remote_entity_names is not OMIT and names_index < len(
                remote_entity_names
            ):
    ...
    def _build_entity_pairs_existing(
        self,
        local_entity,
        remote_entity,
        member_mappings,
        ...
        if not member_mappings:
            if (
                isinstance(local_entity, local_entity.system.cons_groups.object_type)
                and len(local_entity.get_members()) > 0
            ):
                raise InvalidUsageException(
                    "Specifying non-empty remote CG requires passing a `member_mappings` "
                ...
                returned.append(
                    {
                        ...
                        "remote_entity_id": remote_sub_entity.get_parent(
                            from_cache=True
                        ).id,
...
class Replica(SystemObject):
    ...
    FIELDS = [
        ...
        Field(
            "link",
            api_name="link_id",
            binding=RelatedObjectNamedBinding("links"),
        ...
        Field(
            "last_replicated_guid",
            api_name="_consistent_guid",
            is_filterable=True,
            is_sortable=True,
        ),
        ...
        Field(
            "remote_cg_id", type=int, is_filterable=True, is_sortable=True, cached=True
        ),
        ...
        Field(
            "temporary_failure_retry_interval",
            type=MillisecondsDeltaType,
            mutable=True,
            is_filterable=True,
            is_sortable=True,
        ...
        Field(
            ...
            creation_parameter=True,
            mutable=True,
            feature_name="active_active_preferred_on_replica",
        ),
        Field(
            "local_entity",
            api_name="local_entity_id",
            binding=ReplicaEntityBinding(),
            creation_parameter=True,
            optional=True,
            is_filterable=True,
            is_sortable=True,
        ...
        Field(
            ...
            is_sortable=True,
            mutable=False,
            feature_name="concurrent_replication",
        ),
        Field(
            "suspended_from_local",
            type=bool,
            is_filterable=True,
            is_sortable=True,
            mutable=False,
            feature_name="active_active_suspend",
        ),
        Field(
            "including_snapshots",
            type=bool,
    ...
    def _get_entity(self, is_local, from_cache=False, safe=False):
        system = (
            self.system
            if is_local
            else self.get_remote_system(from_cache=from_cache, safe=safe)
        )
        if system is None:
            return None
        ...
        if self.is_consistency_group():
            return system.cons_groups.get_by_id_lazy(
                self.get_field("{}_cg_id".format(side))
            )
    ...
    def _notify_pre_exposure(replica):
        ...
        gossip.trigger_with_tags(
            ...
            {"source": replica.get_local_entity(), "system": replica.system},
            tags=replica._get_entity_tags(),
        )
    ...
    def expose_last_consistent_snapshot(self):
        ...
        try:
            resp = self.system.api.post(
                self.get_this_url_path().add_path("expose_last_consistent_snapshot")
            ).get_result()
    ...
    def get_local_volume(self):
        ...
        if not self.is_volume():
            raise NotImplementedError(
                "get_local_volume() is not supported on a {} replication".format(
    ...
    def get_local_filesystem(self):
        ...
        if not self.is_filesystem():
            raise NotImplementedError(
                "get_local_filesystem() is not supported on a {} replication".format(
                    self.get_entity_type()
                )
            )  # pragma: no cover  # pylint: disable=line-too-long
    ...
    def get_local_cg(self):
        ...
        if not self.is_consistency_group():
            raise NotImplementedError(
                "get_local_cg() is not supported on a {} replication".format(
                    self.get_entity_type()
                )
            )  # pragma: no cover  # pylint: disable=line-too-long
    ...
    def get_remote_data_entity_from_local(self, dataset):
        ...
        if remote_system is None:
            return None
        collection = remote_system.objects.get_binder_by_type_name(
            dataset.get_type_name()
        )
    ...
    def suspend(self):
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.pre_replica_suspend", {"replica": self}, tags=["infinibox"]
        )
        try:
            ...
            with end_reraise_context():
                gossip.trigger_with_tags(
                    "infinidat.sdk.replica_suspend_failure",
                    {"replica": self, "exception": e},
                    tags=["infinibox"],
                )
        gossip.trigger_with_tags(
            "infinidat.sdk.post_replica_suspend", {"replica": self}, tags=["infinibox"]
        )
    ...
    def resume(self):
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.pre_replica_resume", {"replica": self}, tags=["infinibox"]
        )
        try:
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.post_replica_resume", {"replica": self}, tags=["infinibox"]
        )
        self.invalidate_cache("state", "state_reason", "state_description")
    ...
    def switch_role(self):
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.pre_replica_switch_role",
            {"replica": self},
            tags=["infinibox"],
        )
        try:
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.post_replica_switch_role",
            {"replica": self},
            tags=["infinibox"],
        )
    ...
    def _change_type(self, new_type, params=None):
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.pre_replica_change_type",
            {"replica": self, "old_type": old_type, "new_type": new_type},
            tags=["infinibox"],
        )
        try:
            url = (
                self.get_this_url_path().add_path("change_type_to_async")
                if new_type == "async"
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.post_replica_change_type",
            {"replica": self, "old_type": old_type, "new_type": new_type},
            tags=["infinibox"],
        )
        self.invalidate_cache()
    ...
    def _validate_can_check_state(self):
        if self.is_target():
            raise CannotGetReplicaState(
                "Replica state cannot be checked on target replica"
            )
    ...
    def is_idle(self, **kwargs):
        ...
        if self.system.compat.has_sync_job_states():
            return (
                not self.is_replicating()
                and not self.is_initial_replication()
    ...
    def is_initial_replication(self, **kwargs):
        ...
        if self.is_type_active_active():
            return self._is_in_sync_state(
                "initializing", "initializing_pending", **kwargs
            )
        self._validate_can_check_state()
        if self.system.compat.has_sync_job_states():
            return self.is_initial(**kwargs) and self._any_sync_job_state_contains(
    ...
    def is_pending(self, **kwargs):
        ...
        if not self.system.compat.has_sync_job_states():
            raise NotImplementedError(
                'This system ({}) doesn\'t support replica "pending" state'.format(
                    self.system
                )
            )
    ...
    def is_stalled(self, **kwargs):
        ...
        if not self.system.compat.has_sync_job_states():
            raise NotImplementedError(
                "Checking for stalled is not supported on systems without sync job states"
            )
    ...
    def change_role(self, entity_pairs=OMIT):
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.pre_replica_change_role",
            {"replica": self},
            tags=["infinibox"],
        )
        try:
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.post_replica_change_role",
            {"replica": self},
            tags=["infinibox"],
        )
        self.invalidate_cache()
    ...
    def delete(
        ...
        force_if_remote_error=OMIT,
        force_on_target=OMIT,
        force_if_no_remote_credentials=OMIT,
        force_on_local=OMIT,
        keep_serial_on_local=OMIT,
    ):
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.replica_deleted",
            {
                "replica": self,
                "entity_pairs": entity_pairs,
                "deletion_params": path.query_dict,
            },
            tags=["infinibox"],
        )
    ...
    def _get_deletion_result(self, result, remote_replica):
        ...
        if "group" in result["entity_type"].lower():
            ...
            return self._get_local_remote_snapshots(
                result, "cons_groups", remote_replica, "_{0}_reclaimed_sg_id"
            )

        [entity_pair] = result.get("entity_pairs", [None])
        if entity_pair is not None:
            collection_name = (
                "volumes" if result["entity_type"] != "FILESYSTEM" else "filesystems"
            )
            return self._get_local_remote_snapshots(
                entity_pair,
                collection_name,
    ...
    def _get_local_remote_snapshots(
        self, result, collection_name, remote_replica, field_name_template
    ):
        local_reclaimed_id = result.get(field_name_template.format("local"))
        if local_reclaimed_id is None:
            _logger.debug(
                "Could not get local reclaimed id (missing {} in {})",
                field_name_template.format("local"),
                result,
            )
        remote_reclaimed_id = result.get(field_name_template.format("remote"))
        if remote_reclaimed_id is None:
            _logger.debug(
                "Could not get remote reclaimed id (missing {} in {})",
                field_name_template.format("remote"),
                result,
            )
        ...
        if local_reclaimed_id is not None:
            local = getattr(self.system, collection_name).get_by_id_lazy(
                local_reclaimed_id
            )
        if remote_replica is not None and remote_reclaimed_id is not None:
            remote = getattr(remote_replica.system, collection_name).get_by_id_lazy(
                remote_reclaimed_id
            )
        return local, remote
    ...
    def get_remote_replica(self, from_cache=False, safe=False):
        ...
        if linked_system is None:
            return None
        return linked_system.replicas.get_by_id_lazy(
            self.get_remote_replica_id(from_cache=from_cache)
        )
    ...
    def get_remote_entity_pairs(self):
        ...
        return self.system.api.get(
            "remote/{}/api/rest/replicas/{}".format(
                self.get_link().id, self.get_remote_replica_id(from_cache=True)
            )
        ).get_result()["entity_pairs"]
