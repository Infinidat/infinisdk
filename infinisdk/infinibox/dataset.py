...
class Datasets(PolymorphicBinder):
    def __init__(self, system):
        ...
        super(Datasets, self).__init__(
            URL("datasets"), object_types, factory=self._dataset_factory, system=system
        )
    ...
    def _dataset_factory(self, system, received_item):
        ...
        assert dataset_type_str in (
            "VOLUME",
            "FILESYSTEM",
        ), "Unsupported dataset type {}".format(dataset_type_str)
        dataset_binder = system.objects.get_binder_by_type_name(
...
class DatasetTypeBinder(TypeBinder):
    def create_many(self, *args, **kwargs):
        ...
        return [
            self.create(*args, name="{}_{}".format(name, i), **kwargs)
            for i in range(1, count + 1)
        ]

    def calculate_reclaimable_space(self, entities):
        url = URL(self.object_type.get_url_path(self.system)).add_path(
            "delete_simulation"
        )
...
class Dataset(InfiniBoxObject):
    ...
    FIELDS = [
        ...
        Field(
            "qos_policy",
            type="infinisdk.infinibox.qos_policy:QosPolicy",
            api_name="qos_policy_id",
            is_sortable=True,
            is_filterable=True,
            binding=RelatedObjectBinding("qos_policies"),
            feature_name="qos",
            cached=False,
        ),
        Field(
        ...
        Field(
            ...
            creation_parameter=True,
            optional=True,
            feature_name="snapshot_lock",
        ),
        Field("lock_state", type=str, feature_name="snapshot_lock"),
        Field(
            "rmr_active_active_peer",
            type=bool,
            is_sortable=True,
            is_filterable=True,
            feature_name="active_active",
        ),
    ...
    def _get_family_master_id(self):
        assert not self.is_field_supported(
            "family_id"
        ), "Use self.get_family_id(), which adheres to cache policies"
        if self.is_master():
            return self.get_id()
    ...
    def refresh_snapshot(self, force_if_replicated_on_target=OMIT):
        ...
        url = add_normalized_query_params(
            self.get_this_url_path().add_path("refresh"),
    ...
    def _create_child(self, name=None, replicate_to_async_target=OMIT, **kwargs):
        ...
        try:
            ...
            with end_reraise_context():
                gossip.trigger_with_tags(
                    "infinidat.sdk.entity_child_failure",
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.post_entity_child_creation",
    ...
    def _is_synced_remote_entity(self):
        ...
        return (
            source_replica
            and source_replica.is_type_sync()
            and not source_replica.is_async_mode()
        )
    ...
    def trigger_begin_fork(self):
        ...
        if self._is_synced_remote_entity():
            ...
            if remote_entity is not None:
                ...
                _logger.debug(
                    "Could not fetch remote entity for {}. Forking local entity", self
                )
        ...
        hook_tags = self.get_tags_for_object_operations(self.system)
        gossip.trigger_with_tags(
            _BEGIN_FORK_HOOK, {"obj": self._forked_obj}, tags=hook_tags
        )

    def trigger_cancel_fork(self):
        hook_tags = self.get_tags_for_object_operations(self.system)
        gossip.trigger_with_tags(
            _CANCEL_FORK_HOOK, {"obj": self._forked_obj}, tags=hook_tags
        )
    ...
    def trigger_finish_fork(self, child):
        hook_tags = self.get_tags_for_object_operations(self.system)
        gossip.trigger_with_tags(
            _FINISH_FORK_HOOK, {"obj": self._forked_obj, "child": child}, tags=hook_tags
        )
        self._forked_obj = None
    ...
    def trigger_before_restore(self, source):
        hook_tags = self.get_tags_for_object_operations(self.system)
        gossip.trigger_with_tags(
            "infinidat.sdk.pre_object_restore",
            {"source": source, "target": self},
            tags=hook_tags,
        )
        gossip.trigger_with_tags(
            "infinidat.sdk.pre_data_restore",
            {"source": source, "target": self},
            tags=hook_tags,
        )
    ...
    def trigger_restore_failure(self, source, e):
        hook_tags = self.get_tags_for_object_operations(self.system)
        gossip.trigger_with_tags(
            "infinidat.sdk.data_restore_failure",
            {"source": source, "target": self, "exc": e},
            tags=hook_tags,
        )
        gossip.trigger_with_tags(
            "infinidat.sdk.object_restore_failure",
            {"source": source, "target": self, "exc": e},
            tags=hook_tags,
        )
    ...
    def trigger_after_restore(self, source):
        hook_tags = self.get_tags_for_object_operations(self.system)
        gossip.trigger_with_tags(
            "infinidat.sdk.post_data_restore",
            {
                "source": source,
                "target": self,
        ...
        gossip.trigger_with_tags(
            "infinidat.sdk.post_object_restore",
            {"source": source, "target": self},
            tags=hook_tags,
        )
    ...
    def get_replicas(self):
        if (
            isinstance(self, self.system.types.filesystem)
            and not self.system.compat.has_nas_replication()
        ):
            return []
        if (
            self.system.compat.get_version_as_float() >= 5.5
            and isinstance(self, self.system.types.volume)
            and self.is_in_cons_group()
        ):
    ...
    def unassign_qos_policy(self, qos_policy):
        ...
        ), "QoS policy {} is not assigned to {}".format(qos_policy, self)
