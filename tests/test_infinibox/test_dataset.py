...
from ..conftest import create_pool, relevant_from_version, add_internal_volumes_to_system
...
def test_resize_with_force(data_entity):
    is_filesystem = data_entity.get_type_name() == 'filesystem'
    delta = 2 * GB
    initial_size = data_entity.get_size()
    data_entity.resize(delta, force=True)
    expected_size = initial_size + delta
    if is_filesystem:
        expected_size = expected_size.roundup(FILESYSTEM_SIZE_GRANULARITY)
    assert data_entity.get_size() == expected_size
...
def test_replicate_to_async_target_url_param(data_entity):
    snap = data_entity.create_snapshot(replicate_to_async_target=False)
    assert snap.get_parent() == data_entity
...
def test_get_all_internals_dont_filter_by_type(infinibox):
    all_internal_datasets = infinibox.datasets.get_all_internals()
    assert len(all_internal_datasets) == 2
...
def test_get_all_internals_filter_by_type(infinibox):
    master_internal_datasets = infinibox.datasets.get_all_internals(internal_type="master")
    snapshot_internal_datasets = infinibox.datasets.get_all_internals(internal_type="snapshot")

    assert len(master_internal_datasets) == 1
    assert len(snapshot_internal_datasets) == 2


@add_internal_volumes_to_system(number_of_internal_volumes_type_snapshot=20)
def test_get_all_internals_api_pages(infinibox):
    all_internal_datasets_page_1 = infinibox.datasets.get_all_internals(page_size=10, page=1)
    all_internal_datasets_page_2 = infinibox.datasets.get_all_internals(page_size=10, page=2)

    assert len(all_internal_datasets_page_1) == 10
    assert len(all_internal_datasets_page_2) == 10
    assert all_internal_datasets_page_1 != all_internal_datasets_page_2
...
def test_bulk_update_ssa_express_enabled(infinibox, create_many_volumes):
    ...
    infinibox.datasets.bulk_update(entities=volumes, ssa_express_enabled=True)
