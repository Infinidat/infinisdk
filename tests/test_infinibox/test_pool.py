...
def test_pool_capacity_fields_types(pool):
    ...
    #FIXME - REMOVE WHEN EFFECTIVE CAPACITY IS IMPLEMENTED IN INFINISIM 
    excluded_fields = {
        "zeros_capacity",
        "host_written_data",
    ...
    for field in pool.FIELDS:
        ...
        if field_name not in excluded_fields and field_name.endswith('capacity') or field_name.endswith('space'):
...
def test_internal_entities_count_field(infinibox, pool):
    volume = create_volume(infinibox, pool_id=pool.id)
    volume_snapshot = volume.create_snapshot()

    assert pool.get_field('internal_entities_count') == None

    volume_snapshot.promote_snapshot()

    assert pool.get_field('internal_entities_count', from_cache=False) == 1
