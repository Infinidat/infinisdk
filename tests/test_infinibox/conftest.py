...
def cg_replica(infinibox, secondary_infinibox, cg, secondary_pool, link):
    ...
    kwargs = {
        "link": link,
        "remote_pool": secondary_pool,
        "including_snapshots": True,
        "snapshots_retention": 55,
        "lock_remote_snapshot_retention": 50,
    }
...
@pytest.fixture
def active_directory_with_unix_services_enabled(infinibox_with_network_space):
    active_directory = Munch(
        infinibox_with_network_space.active_directory_domains.create(
            domain="AD",
            password="1111",
            preferred_ips=["0.0.0.0"],
            username="user",
