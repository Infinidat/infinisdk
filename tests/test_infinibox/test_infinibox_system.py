...
def test_fields_supported(infinibox, type_binder):
    broken_fields = []
    for field in type_binder.fields:
        try:
            infinibox.is_field_supported(field)
...
def test_block_ssh(infinibox) -> None:
    assert infinibox.close_ssh_ports().status_code == 200
    ports_status = infinibox.get_ssh_ports_status()
    assert ports_status.overall_ssh_port_status == "CLOSED"
    assert infinibox.open_ssh_ports().status_code == 200
    ports_status = infinibox.get_ssh_ports_status()
    assert ports_status.overall_ssh_port_status == "OPEN"

def test_ssa_express_queue_moves_volumes_to_active_state(infinibox, create_many_volumes):
    """
    This test verifies that volumes move from the SSA Express queue to an active state.

    - Initially, 10 volumes are created.
    - The SSA Express feature is enabled on all of these volumes.
    - The test then waits for 1 second to allow the SSA Express queue to process the volumes.
    
    Expected behavior:
    - After enabling SSA Express, 1 volume should be in the "loading" state (actively being processed).
    - The remaining 9 volumes should still be in the SSA Express queue, waiting to be processed.
    
    The assertion checks that the queue contains exactly 9 datasets, confirming that 1 volume has moved to the active state.
    """
    volumes = create_many_volumes(10)
    assert len(infinibox.get_ssa_express_queue_datasets()) == 0
    for volume in volumes:
        volume.enable_ssa_express_enabled()
    flux.current_timeline.sleep(1)
    assert len(infinibox.get_ssa_express_queue_datasets()) == 9

def test_ssa_express_active_datasets_after_moving_them_from_queue(infinibox, create_many_volumes):
    """
    This test verifies that all volumes move from the SSA Express queue to the active state after a sufficient amount of time.

    - Initially, 10 volumes are created.
    - The SSA Express feature is enabled on all of these volumes.
    - The test waits for 100 seconds, which should be enough time for all volumes to move from the queue to the active state.

    Expected behavior:
    - Initially, there should be no active datasets in the SSA Express active state.
    - After enabling SSA Express on the volumes and waiting 100 seconds, all 10 volumes should be in the active state.

    The assertions confirm:
    - Before the volumes are processed, there are 0 active datasets.
    - After the waiting period, all 10 volumes should have moved to the active state.
    """
    volumes = create_many_volumes(10)
