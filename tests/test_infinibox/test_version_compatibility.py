...
def test_version_comparison():
    version = InfiniboxVersion.parse("8.1.0.0-dev")
    assert version.remove_dev() >= "8.1" 
    version = InfiniboxVersion.parse("8.1.0.0-dev")
    assert version < "8.1"
    version = InfiniboxVersion.parse("8.1.0.0")
    assert version >= "8.1" 
    assert not version < "8.1"
    version = InfiniboxVersion.parse("8.0.0.60-dev")
    assert version.remove_dev() >= "8.0.0.50"
    version = InfiniboxVersion.parse("8.0.0.60-dev")
    assert version >= "8.0.0.50"
    version = InfiniboxVersion.parse("8.0.0.60")
    assert version >= "8.0.0.50"
