...
def test_physical_capacity(infinibox):
    ...
    pool_reserve = defs.internal.default_pool_reserved_capacity.roundup(defs.internal.num_vus * defs.internal.section_size)
    assert infinibox.capacities.get_free_physical_capacity() + pool_reserve == infinibox.capacities.get_total_physical_capacity()
