...
def test_disable_enable_network_space_ip(network_space):
    ...
    assert network_space.get_ips() == [
        {'enabled': True, 'ip_address': ip_address, 'reserved': False, 'vlan_id': 1, 'interface_id': interface_id, 'type': 'MANAGMENT'}]
    network_space.disable_ip_address(ip_address)
    assert network_space.get_ips() == [
        {'enabled': False, 'ip_address': ip_address, 'reserved': False, 'vlan_id': 1, 'interface_id': None, 'type': 'MANAGMENT'}]
