...
def test_set_source_identifier(system):
    ...
    assert res.response.request.headers['User-Agent'].startswith('InfiniSDK-Client/')
...
def test_infinidat_header(system):
    res = system.api.get("/api/rest/system")
    assert res.response.request.headers['X-Infinidat-Source'].startswith('InfiniSDK-')

def test_user_agent_header(system):
    res = system.api.get("/api/rest/system")
    assert res.response.request.headers['User-Agent'].startswith('InfiniSDK-Client/')